...
mod tests {
    ...
    fn test_constant_folding_arithmetic() {
        ...
        if let TopLevel::Stmt(Statement::Function { body, .. }) = &folded[0]
    ...
    fn test_algebraic_simplification_add_zero() {
        ...
        // x + 0 should become x
        if let TopLevel::Stmt(Statement::Function { body, .. }) = &simplified[0]
            && let Statement::Return(expr) = &body[0]
        {
    ...
    fn test_combined_optimizations() {
        ...
        if let TopLevel::Stmt(Statement::Function { body, .. }) = &opt[0]
            && let Statement::Return(expr) = &body[0]
        {
            assert_eq!(expr.expr, Expression::Var("x".to_string()));
        }
